...
- **Groq Curator**: configura `GROQ_API_KEY` como variable de entorno del servidor para habilitar respuestas con Groq. Como compatibilidad de despliegue, el backend también acepta `VITE_GROQ_API_KEY` si ya la tenías cargada así en Vercel, aunque sigue siendo preferible `GROQ_API_KEY`. Opcionalmente puedes ajustar `GROQ_MODEL`; por defecto usa `llama-3.3-70b-versatile`. Si la clave no está disponible, el curador cae automáticamente a un modo local basado en reglas para no romper la experiencia.
