...
export const SeriesSectionPage: React.FC = () => {
  const { name } = useParams();
  const sectionName = decodeURIComponent(name ?? '');
  const { visibleMovies, ratings, updateSeen } = useMovies();
  ...
  useEffect(() => {
    setFilters((prev) => ({ ...prev, seccion: sectionName, series: 'series' }));
  }, [sectionName]);
  ...
  const recentIndexMap = useMemo(() => new Map(sectionMovies.map((movie, index) => [movie.id, index])), [sectionMovies]);
  ...
  }, [sectionMovies, filters, ratings, recentIndexMap]);

  const grouped = useMemo(() => groupSeriesForDisplay(sectionMovies, filtered), [sectionMovies, filtered]);
  ...
  useEffect(() => {
    if (!sortOptions.some((option) => option.value === filters.sort)) {
      setFilters((prev) => ({ ...prev, sort: 'title-asc' }));
    }
  }, [filters.sort]);
      ...
      <div className="archive-filter-bar">
        <div className="archive-filter-row archive-filter-row--top">
          ...
          <ArchiveDropdown
            ...
            value={filters.sort}
            options={sortOptions}
            onChange={(value) => setFilters({ ...filters, sort: value as MovieFilters['sort'] })}
        ...
        <div className="archive-filter-row archive-filter-row--bottom">
          <ArchiveDropdown
            ...
            onChange={(value) => setFilters({ ...filters, seccion: value || null })}
          ...
          <ArchiveDropdown
            ...
            onChange={(value) => setFilters({ ...filters, saga: value || null })}
          ...
          <ArchiveDropdown
            ...
            onChange={(value) => setFilters({ ...filters, series: value as MovieFilters['series'] })}
          ...
          <ArchiveDropdown
            ...
            onChange={(value) => setFilters({ ...filters, seen: value as MovieFilters['seen'] })}
            placeholder="All"
          />
          {hasActiveFilters && (
